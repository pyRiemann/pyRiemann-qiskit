...
this module. If it is not installed, this module still imports cleanly and
the classes below can still be introspected (they are documented in the API
reference either way); only instantiating one raises `ImportError`.
...
# The classes below are defined whether or not p-kit is importable, and
# check `HAS_PKIT` at construction instead: guarding their *definition* on
# it makes them vanish from an environment without p-kit, which is what the
# documentation build is -- p-kit pins numpy, scipy and cvxpy versions that
# conflict with `doc/requirements.txt` -- so autosummary then fails to
# import them and their API pages are never generated.
...
class _PBitOptimizer(pyQiskitOptimizer):
    ...
    def __init__(
        ...
        if not HAS_PKIT:
            raise ImportError(
                f"p-kit is required for {type(self).__name__}. "
                "Install it with "
