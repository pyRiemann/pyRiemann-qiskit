...
mean ROC AUC (0.622), confirming the strength of Riemannian geometry for
resting-state EEG.  QIOCE (0.601) and \textsc{QuantumStateDiscriminator}
(0.600) are virtually tied for second, with QSD being the fastest classifier
tested (under 0.1~s mean training time) versus $\approx$6~min for QIOCE.
NCH-based quantum methods trail classical baselines; transfer learning
has mixed effects depending on the pipeline.
...
\begin{table}[h]
  ...
  \begin{tabular}{lcc}
    ...
    \midrule
    \textbf{MDM}                             & \textbf{0.622} & \textbf{2.1} \\
    QIOCE                                   & 0.601          & 361.7 \\
    QuantumStateDiscriminator               & 0.600          & 0.08 \\
    MDWM(0.5)                               & 0.580          & 8.2 \\
    MDM+TL                                  & 0.578          & 63.6 \\
    TS+LR+TL                                & 0.577          & 80.3 \\
    QIOCE+TL                                & 0.569          & 570.5 \\
    TS+LR                                   & 0.563          & 15.1 \\
    NCH+QAOACV(Ulvi)                        & 0.555          & 1.8 \\
    NCH+QAOACV(Ulvi)+TL                     & 0.551          & 117.6 \\
    NCH+NaiveQAOA                           & 0.538          & 1.8 \\
    NCH+NaiveQAOA+TL                        & 0.538          & 116.9 \\
    \bottomrule
  \end{tabular}
\end{table}

Figure~\ref{fig:scatter} shows the distribution of per-subject AUC scores
across all three datasets.  MDM achieves the highest mean performance.
QIOCE (0.601) and \textsc{QuantumStateDiscriminator} (0.600) are virtually
tied for second.  NCH variants cluster near the lower end, with all
pipelines remaining above chance level.
...
\begin{figure}[h]
  \centering
  \includegraphics[width=\columnwidth]{roc_auc_per_dataset.png}
...
\begin{figure}[h]
  \centering
  \includegraphics[width=\columnwidth]{results_log.png}
...
Transfer learning has mixed effects depending on the pipeline.
For MDM, TL is detrimental ($0.622 \to 0.578$), suggesting that RPA
alignment disrupts the Riemannian mean structure that MDM relies upon.
By contrast, TS+LR benefits from TL ($0.563 \to 0.577$), and QIOCE
degrades with TL ($0.601 \to 0.569$), suggesting the whitening and
tangent-space projection in QIOCE already provides implicit alignment.
NCH+NaiveQAOA shows virtually no effect of TL ($0.538 \to 0.538$),
while the Ulvi variant degrades slightly ($0.555 \to 0.551$).
These results confirm that RPA alignment benefits tangent-space linear
classifiers but is neutral or harmful for distance-based and quantum
circuit methods.
...
The \textsc{QuantumStateDiscriminator} achieves a mean AUC of 0.600, placing
it virtually tied with QIOCE (0.601) and making it the fastest classifier
overall (0.08~s mean training time)---over four orders of magnitude faster
than QIOCE ($\approx$362~s) and two orders faster than MDM (2.1~s).
Both QSD and QIOCE fall below MDM (0.622), but neither the gap nor the
difference between the two quantum methods is likely to be statistically
significant given the per-dataset convergence visible in
...
RPA alignment has classifier-dependent effects.  It degrades MDM
($0.622 \to 0.578$), likely because centering and rotating the covariance
manifold disrupts the class-mean geometry that MDM relies on.  Conversely,
TS+LR benefits from alignment ($0.563 \to 0.577$): projecting aligned
covariances to the tangent space yields more separable features for logistic
regression.  QIOCE and NCH+NaiveQAOA also benefit marginally.  These results
suggest that RPA is most useful when downstream classifiers operate in tangent
or Euclidean space, and less so for pure Riemannian distance classifiers.
...
The best overall configuration is MDM (AUC 0.622, 2.1~s), confirming
that classical Riemannian geometry remains a strong baseline.
QIOCE (0.601, $\approx$6~min) and \textsc{QuantumStateDiscriminator}
(0.600, 0.08~s) are virtually tied for second; QSD achieves competitive
accuracy at a fraction of the computational cost.
NCH variants cluster around 0.538--0.555.
...
yet closed.  Closing this gap is the primary open challenge; Riemannian
transfer learning has mixed effects and does not systematically bridge it.
