...
\begin{itemize}
  ...
  \item \textbf{MDM $>$ TS+LDA+TL} ($p=0.02$, SMD$=1.45$): MDM without
    transfer learning outperforms TS+LDA with full RPA alignment.
  \item \textbf{MDM+TL $>$ TS+LDA+TL} ($p=0.04$, SMD$=1.77$): after RPA
...
($0.529 \to 0.517$), and TS+LDA drops from $0.584$ to $0.542$.
...
($p=0.04$, SMD$=1.77$) confirms that after alignment MDM is the more
robust classifier.  The Ulvi NCH variant with TL also remains below MDM
($p=0.04$), suggesting that neither the quantum circuit improvements nor
