...
The dataset contains 60 trials ($8$ channels, $100$ time points, sampled
at 256~Hz) and is evaluated with stratified 5-fold cross-validation.

We perform a full grid search over 48 configurations:
6 kernel operators (gaussian, helmholtz, cauchy, gabor, laplacian,
mexican\_hat) $\times$ 2 Tikhonov regularization strengths
($\lambda \in \{0.01, 0.1\}$) $\times$ 2 spatial filter ranks
($K \in \{8, 16\}$) $\times$ 2 temporal basis sizes ($R \in \{32, 64\}$),
for a total of 240 fits.
...
The \textbf{mexican\_hat} (band-pass) and \textbf{gaussian} kernels lead
with mean AUC $\approx 0.62$, while gabor and laplacian trail at $\approx 0.59$.
The strong performance of mexican\_hat is consistent with the data generation:
the band-pass shape of the kernel naturally resonates with the amplitude
differences between alpha and beta bands.
Contrary to initial expectation, the gabor kernel---which is designed for
frequency-localized bursts---does not outperform gaussian, suggesting that
on this stationary sinusoidal data the frequency localization offered by
gabor's Gaussian envelope is not necessary.
...
\begin{figure}[h]
  ...
  \caption{Kernel operator comparison on synthetic sinusoidal EEG
    (averaged over $\lambda$, $K$, $R$; 5-fold CV, $n=60$ trials).
    Dotted line: LinearHead baseline (mean AUC $= 0.617$).
    Error bars: $\pm 1$ SD across folds.}
  \label{fig:ablation_igl_operator}
\end{figure}

\paragraph{Effect of $\lambda$, $K$, and $R$.}
Figure~\ref{fig:ablation_igl_heatmaps} (left) shows the operator~$\times$~$\lambda$
interaction.  Higher regularization ($\lambda=0.1$) substantially benefits
peaked kernels: mexican\_hat gains $+0.081$ (0.590~$\to$~0.671) and cauchy
gains $+0.075$ (0.561~$\to$~0.636), while gaussian mildly prefers lower
regularization ($-0.016$).  This reflects the different conditioning of the
least-squares solve: peaked kernels produce sparser Gram matrices that
benefit from stronger Tikhonov damping.

Figure~\ref{fig:ablation_igl_heatmaps} (right) shows the $K$~$\times$~$R$
interaction.  A larger spatial filter bank ($K=16$, mean AUC~$0.637$) is
consistently better than $K=8$ (mean AUC~$0.571$), which is expected given
the random mixing matrix that spreads class information across all 8 channels.
The number of temporal anchors $R$ has a weaker effect; $R=32$ is slightly
preferable for $K=16$ (0.655 vs.\ 0.619), while for $K=8$ the two are
near-equivalent.
...
\begin{figure}[h]
  ...
  \caption{Hyperparameter heatmaps for IGL on synthetic EEG.
    ...
    All values are mean ROC AUC from 5-fold CV.}
...
(mexican\_hat, $\lambda=0.1$, $K=8$, $R=64$; AUC~$=0.756$) against the
LinearHead baseline (AUC~$=0.617$).
The best IGL configuration outperforms mean-pool logistic regression by
$+0.14$ AUC, demonstrating that the Green kernel temporal integration
captures discriminative temporal structure that simple mean-pooling discards.
Both methods show high fold-to-fold variance owing to the small dataset
($n=60$), but the IGL confidence interval is entirely above the LinearHead
mean, supporting the advantage of the kernel approach on oscillatory EEG.

\begin{figure}[h]
  \centering
  \includegraphics[width=\columnwidth]{igl_ablation_study_bestigl_vs_linearhead.png}
  \caption{Best IGL configuration (mexican\_hat, $\lambda=0.1$, $K=8$,
    $R=64$) vs.\ LinearHead baseline (mean-pool $+$ logistic regression).
    Strip: per-fold AUC scores.  Point: mean $\pm$ SD\@.
    Dotted line: LinearHead mean (0.617).}
  \label{fig:ablation_igl_vs_linear}
\end{figure}

Overall, the ablation suggests that (i)~band-pass kernels (mexican\_hat)
are well-matched to oscillatory EEG data; (ii)~stronger Tikhonov
regularization benefits peaked kernels; (iii)~a wider spatial filter bank
($K=16$) is important when channel mixing is strong; and (iv)~the
Green kernel temporal integration meaningfully outperforms a linear baseline,
validating the IGL temporal aggregation mechanism on sinusoidal EEG.
