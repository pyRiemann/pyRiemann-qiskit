...
for n_hulls_per_class in range(1, max_hull_per_class + 1, 1):
    for n_samples_per_hull in range(1, max_hull_per_class + 1, samples_step):

        print(f"NCH+MIN_HULL_{n_hulls_per_class}h_{n_samples_per_hull}samples")
        pipelines[f"NCH+MIN_HULL_{n_hulls_per_class}h_{n_samples_per_hull}samples"] = \
         make_pipeline(
            sf,
            QuanticNCH(
                seed=seed,
...
if strategy == "min":
    ...
    ax.plot_trisurf(means.n_hull, means.n_samples, means.score, cmap=cm.jet, linewidth=0.2)
